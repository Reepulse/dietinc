	...
	class WPEX_Recent_Posts_Thumb_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				$name = __( 'Chic - Posts With Thumbnails', 'chic' ),
				array(
					'description'   => __( 'Recent posts with thumbnails.', 'chic' ),
				)
			);
		}
				...
				<ul class="wpex-widget-recent-posts widget-recent-list wpex-clr">
					...
					<?php foreach( $wpex_query->posts as $post ) : setup_postdata( $post );
						...
						if ( has_post_thumbnail() ) { ?>
							...
							<li>
								<a href="<?php the_permalink(); ?>" title="<?php wpex_esc_title(); ?>" class="wpex-thumbnail wpex-clr">
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e( 'Order', 'chic' ); ?>:</label>
				<br />
				<select class='wpex-select' name="<?php echo $this->get_field_name( 'order' ); ?>" id="<?php echo $this->get_field_id( 'order' ); ?>">
				<option value="DESC" <?php if( $order == 'DESC' ) { ?>selected="selected"<?php } ?>><?php _e( 'Descending', 'chic' ); ?></option>
				<option value="ASC" <?php if( $order == 'ASC' ) { ?>selected="selected"<?php } ?>><?php _e( 'Ascending', 'chic' ); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Order By', 'chic' ); ?>:</label>
				<br />
				...
				<?php $orderby_array = array (
					'date'          => __( 'Date', 'chic' ),
					'title'         => __( 'Title', 'chic' ),
					'modified'      => __( 'Modified', 'chic' ),
					'author'        => __( 'Author', 'chic' ),
					'rand'          => __( 'Random', 'chic' ),
					'comment_count' => __( 'Comment Count', 'chic' ),
				);
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Category', 'chic' ); ?>:</label>
				<br />
				<select class='wpex-select' name="<?php echo $this->get_field_name( 'category' ); ?>" id="<?php echo $this->get_field_id( 'category' ); ?>">
				<option value="all" <?php if($category == 'all' ) { ?>selected="selected"<?php } ?>><?php _e( 'All', 'chic' ); ?></option>
				<?php
			...
			<?php /* Image Size */ ?>
			<p>
				<label for="<?php echo $this->get_field_id( 'img_size' ); ?>"><?php _e( 'Image Size', 'chic' ); ?></label>
				<br />
