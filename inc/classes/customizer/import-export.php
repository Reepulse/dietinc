	...
	function wpex_customizer_options_sanitize( $options ) {
		...
		if ( $options ) {
			...
			if ( '-1' == $options['reset'] ) {
				...
				// Error messages
				$error_msg = __( 'All theme customizer settings have been reset.', 'chic' );
				$error_type = 'updated';
			}
			...
			elseif( ! empty( $options['import'] ) ) {
				...
				if ( '0' == json_last_error() ) {
					...
					$error_msg  = __( 'Theme customizer settings imported successfully.', 'chic' );
					$error_type = 'updated';
				}
				// Display invalid json data error
				else {
					$error_msg  = __( 'Invalid Import Data.', 'chic' );
					$error_type = 'error';
				}
			}
			// No json data entered
			else {
				$error_msg = __( 'No import data found.', 'chic' );
				$error_type = 'error';
			}
...
if ( ! function_exists( 'wpex_customizer_options_page' ) ) {
	function wpex_customizer_options_page() { ?>
		<div class="wrap">
		<h2><?php _e( 'Import, Export or Reset Customizer Settings', 'chic' ); ?></h2>
		<?php
		// Default options
		$options = array(
				'import'	=> '',
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php _e( 'Export Customizer Options', 'chic' ); ?></th>
					<td>
						<?php
						...
						if ( $theme_mods = get_theme_mods() ) {
							...
							$json = __( 'No Options Found', 'chic' );
							$disabled = 'disabled';
						}
						echo '<textarea rows="10" cols="50" disabled id="wpex-customizer-export" style="width:100%;">' . $json . '</textarea>'; ?>
						<p class="submit">
							<a href="#" class="button-primary wpex-highlight-options <?php echo $disabled; ?>"><?php _e( 'Highlight Options', 'chic' ); ?></a>
						</p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e( 'Import Customizer Options', 'chic' ); ?></th>
					<td>
						<?php
						// Get import data
						...
						<p class="submit">
							<input type="submit" class="button-primary wpex-submit-form" value="<?php _e( 'Import Options', 'chic' ) ?>" />
							<a href="#" class="button-secondary wpex-delete-options"><?php _e( 'Reset Options', 'chic' ); ?></a>
							<a href="#" class="button-secondary wpex-cancel-delete-options" style="display:none;"><?php _e( 'Cancel Reset', 'chic' ); ?></a>
						</p>
						<div class="wpex-delete-options-warning error inline" style="display:none;">
							<p style="margin:.5em 0;"><?php _e( 'Always make sure you have a backup of your settings before resetting, just incase!', 'chic' ); ?></p>
						</div>
					</td>
				</tr>
			</table>
			...
			(function($) {
				"use strict";
					...
					$('.wpex-delete-options').click( function() {
						...
						$('.wpex-submit-form').val( "<?php _e( 'Confirm Reset', 'chic' ); ?>" );
					...
					$('.wpex-cancel-delete-options').click( function() {
						...
						$('.wpex-submit-form').val( "<?php _e( 'Import Options', 'chic' ); ?>" );
